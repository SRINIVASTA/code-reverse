...
st.markdown('<div class="brand-title">🔄 Local Prompt Compiler (No API Needed)</div>', unsafe_allowed_html=False)
st.markdown('<div class="tagline">Turn any public GitHub URL into a structured system prompt instantly.</div>', unsafe_allowed_html=False)

# 2. Main Search Form Bar Input Field
...
owner, repo = extract_repo_details(url_bar_input)

# 4. Local Compiler Button (Runs completely offline/without keys)
if st.button("Get Prompt Blueprint", type="primary", use_container_width=True):
    if owner and repo:
        with st.spinner("Parsing target metadata and generating blueprint..."):
            
            # Formulate a structured prompt using fixed template variables instead of an AI engine
            compiled_prompt = f"""You are an elite software engineering agent and system architect. Your objective is to recreate the complete, foundational system architecture of the project '{repo}' owned by '{owner}' from scratch.
...
- **Tech Stack Identification:** Analyze the underlying codebase configuration to structure clean HTML5 layout templates, presentational logic scripts, and modern view interactive hooks.
...
"""
            ...
            # Print the generated text inside the browser view
